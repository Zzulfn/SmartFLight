...
void aws_iot_task(void *param) {
...
#if 1
	jsonStruct_t Led_Station;
    Led_Station.cb = NULL;
    Led_Station.pKey = "color";
    Led_Station.pData =&clor;
    Led_Station.type = SHADOW_JSON_UINT8;
    Led_Station.dataLength = sizeof(u8_t);
#endif
    ...
    ESP_LOGI(TAG, "Shadow Connect pMyThingName:%s pMqttClientId:%s ",CONFIG_AWS_EXAMPLE_THING_NAME,CONFIG_AWS_EXAMPLE_CLIENT_ID);
    ...
    while(NETWORK_ATTEMPTING_RECONNECT == rc || NETWORK_RECONNECTED == rc || SUCCESS == rc) {
        ...
        if(SUCCESS == rc) {
          ...
          //                              &windowActuator,&Led_Station);
			
			//rc = aws_iot_shadow_add_reported(JsonDocumentBuffer, sizeOfJsonDocumentBuffer, 2,&windowActuator,&temperatureHandler);
		    rc = aws_iot_shadow_add_reported(JsonDocumentBuffer, sizeOfJsonDocumentBuffer, 1,&Led_Station);
            if(SUCCESS == rc) {
